...
export default function CarsTab({ cars, onUpdate, onAdd, onDelete }: Props) {
  ...
  const [categoryFilters, setCategoryFilters] = useState<Record<string, "all" | "available" | "unavailable">>({});
  const [categoryBookingFilters, setCategoryBookingFilters] = useState<Record<string, "all" | "pending" | "completed" | "cancelled">>({}); 
  // ============================================
  ...
  const t = {
    ...
    filterAvailable: isEnglish ? "Available" : "متاح",
    filterUnavailable: isEnglish ? "Unavailable" : "غير متاح",
  ...
  // Helper functions
const getCatFilter = (cat: string) => categoryFilters[cat] ?? "all";
const setCatFilter = (cat: string, val: "all" | "available" | "unavailable") => {
  setCategoryFilters(prev => ({ ...prev, [cat]: val }));
};

const getCatBookingFilter = (cat: string) => categoryBookingFilters[cat] ?? "all";
const setCatBookingFilter = (cat: string, val: "all" | "pending" | "completed" | "cancelled") => {
  setCategoryBookingFilters(prev => ({ ...prev, [cat]: val }));
};
    ...
    <div className={language === "ar" ? "rtl" : "ltr"}>
      ...
      {/* <div className="flex items-center gap-2 mb-6">
        {(["all", "available", "unavailable"] as const).map((f) => (
          ...
          >
            {f === "all" ? t.filterAll : f === "available" ? t.filterAvailable : t.filterUnavailable}
          </button>
        ))}
      </div> */}
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                ...
                <ImageUploadButton small={false} onUpload={(url) => setNewCar({ ...newCar, image: url })} />
              </div>
      ...
      {/* ✅ CAR CARDS - Category wise with booking summary */}
      {categories.map((cat) => {

         const catFilter = getCatFilter(cat);
         const bookingFilter = getCatBookingFilter(cat);

        // ✅ Category filtered cars
        // const catCars = cars
        //   .filter((c) => (isEnglish ? c.categoryEn || c.category : c.category) === cat)
        //   .filter((c) =>
        //     availFilter === "all" ? true : availFilter === "available" ? c.available : !c.available
        //   );
         const catCars = cars
    .filter((c) => (isEnglish ? c.categoryEn || c.category : c.category) === cat)
    .filter((c) =>
      catFilter === "all" ? true : catFilter === "available" ? c.available : !c.available
    )
    .filter((c) => {
      if (bookingFilter === "all") return true;
      if (bookingFilter === "pending") return (c.bookingSummary?.pending || 0) > 0;
      if (bookingFilter === "completed") return (c.bookingSummary?.completed || 0) > 0;
      if (bookingFilter === "cancelled") return (c.bookingSummary?.cancelled || 0) > 0;
      return true;
    });

        // ✅ Category level booking summary
        const catSummary = catCars.reduce(
          (acc, car) => {
            acc.pending += car.bookingSummary?.pending || 0;
            acc.confirmed += car.bookingSummary?.confirmed || 0;
            acc.completed += car.bookingSummary?.completed || 0;
            acc.cancelled += car.bookingSummary?.cancelled || 0;
            return acc;
          },
        ...
        return (
          <div key={cat} className="mb-8">
            ...
            {/* ✅ Category Booking Summary Cards */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
              {/* Total Cars */}
              <button
                onClick={() => setCatBookingFilter(cat, "all")}
                className={`bg-[hsl(22,85%,18%)]/10 border-2 rounded-xl px-4 py-3 flex items-center gap-3 transition-all cursor-pointer ${
                  bookingFilter === "all"
                    ? "border-[hsl(22,85%,18%)] shadow-sm"
                    : "border-[hsl(22,85%,18%)]/20 hover:border-[hsl(22,85%,18%)]/40"
                }`}
              >
                <span className="text-2xl">📋</span>
                <div>
                  <p className="text-xl font-black text-[hsl(22,85%,18%)]">
                    {catCars.filter(c => c.available).length}
                    <span className="text-sm font-semibold text-gray-400">
                      /{catCars.length}
                    </span>
                  </p>
                  <p className="text-[11px] text-[hsl(22,85%,30%)] font-semibold">
                    {isEnglish ? "Available / Total" : "متاح / الإجمالي"}
                  </p>
                </div>
              </button>
              {/* Pending */}
              <button
                onClick={() => setCatBookingFilter(cat, "pending")}
                className={`bg-yellow-50 border-2 rounded-xl px-4 py-3 flex items-center gap-3 transition-all cursor-pointer ${
              ...
              >
                <span className="text-2xl">🟡</span>
                <div>
                  <p className="text-xl font-black text-yellow-600">{catSummary.pending}</p>
                  <p className="text-[11px] text-yellow-500 font-semibold">{t.pending}</p>
                </div>
              </button>
              {/* Completed */}
              <button
                onClick={() => setCatBookingFilter(cat, "completed")}
                className={`bg-green-50 border-2 rounded-xl px-4 py-3 flex items-center gap-3 transition-all cursor-pointer ${
              ...
              >
                <span className="text-2xl">✅</span>
                <div>
                  <p className="text-xl font-black text-green-600">{catSummary.completed}</p>
                  <p className="text-[11px] text-green-500 font-semibold">{t.completed}</p>
                </div>
              </button>
              {/* Cancelled */}
              <button
                onClick={() => setCatBookingFilter(cat, "cancelled")}
                className={`bg-red-50 border-2 rounded-xl px-4 py-3 flex items-center gap-3 transition-all cursor-pointer ${
              ...
              >
                <span className="text-2xl">❌</span>
                <div>
                  <p className="text-xl font-black text-red-600">{catSummary.cancelled}</p>
                  <p className="text-[11px] text-red-500 font-semibold">{t.cancelled}</p>
                </div>
              </button>
            </div>
                ...
                >
                  <div className="h-32 bg-[hsl(30,15%,96%)] overflow-hidden relative">
                    ...
                    {confirmDelete === car.id ? (
                      <div className="absolute top-2 right-2 flex items-center gap-1 bg-white rounded-lg shadow-lg px-2 py-1">
                        ...
                        <button onClick={() => setConfirmDelete(null)} className="text-gray-400 text-[10px] px-1">
                          {t.no}
                        </button>
                      </div>
                    ) : (
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-gray-400 mb-3">
                      {car.year} • {car.seats} {t.seatsLabel} • {car.transmission}
                    </p>
                    ...
                    {car.features && car.features.length > 0 && (
                      <div className="flex flex-wrap gap-1 mb-3">
                        {car.features.map((f, i) => (
                          <span
                            key={i}
                            className="flex items-center gap-1 bg-[hsl(30,15%,96%)] text-gray-600 text-[10px] px-2 py-0.5 rounded-md"
                          >
            ...
            {/* ✅ No cars message for this category */}
            {catCars.length === 0 && (
              <div className="text-center py-8 text-gray-400 text-sm">
                {isEnglish ? "No cars found in this filter" : "لا توجد سيارات في هذا الفلتر"}
              </div>
            )}
          </div>
        );
