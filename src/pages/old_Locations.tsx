...
function buildEmbedUrl(lat: number, lng: number, mapUrl?: string, mapEmbed?: string): string {
  // رابط التضمين المخصص من لوحة التحكم — الأولوية الأولى
    ...
    <div className="pt-16 md:pt-20">
      <section className="bg-[hsl(22,85%,18%)] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          >
            {allLocations.length} {language === "ar" ? "فرع في" : "branches in"} {cities.length} {language === "ar" ? "مدن — نخدمك قريباً منك دائماً" : "cities — we serve you nearby"}
          </motion.p>
        </div>
      </section>
          ...
          {cities.map((city) => (
            <div key={city}>
              ...
              >
                ...
                <span className="text-sm text-gray-400">{grouped[city].length} {grouped[city].length === 1 ? "فرع" : "فروع"}</span>
              </motion.div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2">
                      ...
                      <div className="p-8 lg:p-10">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            <h3 className="text-xl font-black text-[hsl(22,85%,18%)]">{localized(loc, "branch", language)}</h3>
                            {loc.isMain && (
                              <span className="text-[hsl(40,88%,44%)] text-xs font-bold bg-[hsl(40,88%,95%)] px-2 py-0.5 rounded-full">
                                {language === "ar" ? "المقر الرئيسي" : "Main Branch"}
                              </span>
                            )}
                          </div>
                        </div>

                        <p className="text-gray-500 text-sm leading-relaxed mb-5">{localized(loc, "description", language)}</p>

                        <div className="space-y-3 mb-6">
                          <div className="flex items-center gap-3">
                            <MapPin size={16} className="text-[hsl(40,88%,48%)] shrink-0" />
                            <span className="text-[hsl(22,85%,18%)] font-medium text-sm">{localized(loc, "address", language)}</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <Phone size={16} className="text-[hsl(40,88%,48%)] shrink-0" />
                            <a href={`tel:${loc.phone}`} className="text-[hsl(22,85%,18%)] font-medium text-sm hover:text-[hsl(40,88%,44%)] transition-colors">
                              {loc.phone}
                            </a>
                          </div>
                          <div className="flex items-center gap-3">
                            <Clock size={16} className="text-[hsl(40,88%,48%)] shrink-0" />
                            <span className="text-[hsl(22,85%,18%)] font-medium text-sm">{localized(loc, "hours", language)}</span>
                          </div>
                        </div>

                        <div>
                          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">الخدمات المتاحة</p>
